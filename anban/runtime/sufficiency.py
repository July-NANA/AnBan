...
_SYSTEM_INSTRUCTIONS = (
    ...
    "Skill is an implementation path, not the new target Skill. activate_skill means the selected "
    "ready Skill itself can satisfy the original goal; never use activate_skill for a guide whose "
    "purpose is only to discover or install a different target. This remains true when that guide "
    "will be the first Tool Call after the acquire_skill decision. Set "
