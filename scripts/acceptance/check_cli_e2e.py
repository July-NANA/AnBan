...
async def gate_bcd(root: Path) -> dict[str, object]:
    ...
    install = await submit(
        ...
        "I explicitly authorize discovering, reviewing, and installing exactly one new target "
        "Skill from the public Skill registry. Any ready discovery or installation helper is only "
        "an implementation guide: it cannot perform the requested transformation and must not be "
        "treated as an already-sufficient target workflow. First record the governed decision "
        "that the absent target Skill must be acquired, then use the applicable ready acquisition "
        "path to run exactly one public search for an uppercase-text Skill implemented by a "
        "bundled local Python or shell script, without logging in. Compare "
        ...
        "After the real install, discover and activate that exact new Skill in this same Run, "
        "follow its "
